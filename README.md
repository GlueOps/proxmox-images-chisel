...
- `docker.io/jpillora/chisel:1` — preloaded from a baked-in tarball on first boot (no registry pull at provision time)
...
The build runs on a stock GitHub runner using `virt-customize`/`virt-sparsify` (libguestfs) — no packer or self-hosted KVM runner required.
